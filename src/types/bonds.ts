...
export interface MatchReason {
  type: 'mutual_friends' | 'same_style' | 'similar_time' | 'streak_match' | 'location' | 'activity_level' | 'mood_match'
  score: number
  detail?: string
}
...
export const BOND_TYPE_INFO: Record<BondType, { label: string; emoji: string; description: string }> = {
  dance_buddy: {
    label: 'Dance Buddy',
    emoji: '🕺',
