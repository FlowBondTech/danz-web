...
export default function MappPage() {
  ...
  useEffect(() => {
    ...
    if (!farcasterLoaded || !privyReady) return
  ...
  }, [farcasterLoaded, privyReady, isAuthenticated, user, authLoading, farcasterReady])
      ...
      <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-b from-bg-primary to-[#0f0f1e] p-6">
        ...
        {!isInFrame && (
          <p className="text-text-muted text-sm mt-4">
            Connect with wallet, email, or social
          </p>
        )}
      </div>
    )
  ...
  return (
    <div className="flex flex-col h-screen bg-bg-primary">
      ...
      <nav className="flex items-center justify-around px-2 py-2 border-t border-white/10 bg-bg-secondary/90 backdrop-blur-md safe-area-bottom">
        ...
        <NavButton icon="dance" label="Dance" active={activeTab === 'dance'} onClick={() => setActiveTab('dance')} />
        <NavButton icon="events" label="Events" active={activeTab === 'events'} onClick={() => setActiveTab('events')} />
...
function NavButton({
  ...
  const icons: Record<string, string> = {
    ...
    dance: 'M9 19V6l12-3v13M9 19c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zm12-3c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zM9 10l12-3',
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="bg-gradient-to-br from-neon-pink/20 to-neon-purple/20 rounded-2xl p-6 border border-white/10">
        ...
        <p className="text-text-secondary text-sm mb-4">Start a session to earn tokens and track your moves.</p>
        <button
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <button className="py-3 bg-white/10 rounded-xl font-semibold text-text-primary">Withdraw</button>
      </div>
    </div>
  )
}
      ...
      <div className="bg-bg-secondary rounded-2xl p-6 border border-white/5 text-center">
        <div className="w-20 h-20 mx-auto mb-4 rounded-full bg-gradient-to-br from-neon-pink to-neon-purple flex items-center justify-center">
          {user?.avatar_url ? (
            ...
            <span className="text-3xl text-white font-bold">{user?.username?.[0]?.toUpperCase() || 'D'}</span>
          )}
        </div>
        <h2 className="text-xl font-bold text-text-primary">{user?.display_name || user?.username || 'Dancer'}</h2>
